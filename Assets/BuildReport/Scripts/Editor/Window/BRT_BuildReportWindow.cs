...
public class BRT_BuildReportWindow : EditorWindow
{
	...
	void InitGUISkin()
	{
		string guiSkinToUse = Settings.DEFAULT_GUI_SKIN_FILENAME;
	...
	void DrawTopRowButtons()
	{
		...
		if (GUI.Button(new Rect(toolbarX, 5, 50, 40), _toolbarLabelLog, Settings.TOOLBAR_LEFT_STYLE_NAME) && !LoadingValuesFromThread)
		{
			Refresh();
		}
		toolbarX += 50;
		if (GUI.Button(new Rect(toolbarX, 5, 40, 40), _toolbarLabelOpen, Settings.TOOLBAR_MIDDLE_STYLE_NAME) && !LoadingValuesFromThread)
		{
		...
		if (GUI.Button(new Rect(toolbarX, 5, 40, 40), _toolbarLabelSave, Settings.TOOLBAR_RIGHT_STYLE_NAME) && BuildReportTool.Util.BuildInfoHasContents(_buildInfo))
		{
		...
		//if (!BuildReportTool.Util.BuildInfoHasContents(_buildInfo))
		{
			if (GUI.Button(new Rect(toolbarX, 5, 55, 40), _toolbarLabelOptions, Settings.TOOLBAR_LEFT_STYLE_NAME))
			{
				_selectedCategoryIdx = OPTIONS_IDX;
			}
			toolbarX += 55;
			if (GUI.Button(new Rect(toolbarX, 5, 70, 40), _toolbarLabelHelp, Settings.TOOLBAR_RIGHT_STYLE_NAME))
			{
	...
	void OnGUI()
	{
		...
		GUI.Label(new Rect(0, 0, position.width, 20), BuildReportTool.Info.ReadableVersion, Settings.VERSION_STYLE_NAME);
		...
		if (GUILayout.Toggle(IsInOverviewCategory, "Overview", Settings.TAB_LEFT_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = OVERVIEW_IDX;
		}
		if (GUILayout.Toggle(IsInBuildSettingsCategory, "Project Settings", Settings.TAB_MIDDLE_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = BUILD_SETTINGS_IDX;
		}
		if (GUILayout.Toggle(IsInSizeStatsCategory, "Size Stats", Settings.TAB_MIDDLE_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = SIZE_STATS_IDX;
		}
		if (GUILayout.Toggle(IsInUsedAssetsCategory, "Used Assets", Settings.TAB_MIDDLE_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
			_selectedCategoryIdx = USED_ASSETS_IDX;
		}
		if (GUILayout.Toggle(IsInUnusedAssetsCategory, "Unused Assets", Settings.TAB_RIGHT_STYLE_NAME, GUILayout.ExpandWidth(true)))
		{
